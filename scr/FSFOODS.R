...
# Converting FSFOODS to a factor for better visualization in plots
cps_data = cps_data %>% 
  mutate(FSFOODS_factor = as.factor(FSFOODS))

# Creating bar plots and histograms to explore relationships between X variables and FSFOODS (food insecurity)
...
lasso_rocCurve = roc(response = as.factor(test.df.preds$FSFOODS), predictor = test.df.preds$lasso_pred, levels = c("0", "1"))
ridge_rocCurve = roc(response = as.factor(test.df.preds$FSFOODS), predictor = test.df.preds$ridge_pred, levels = c("0", "1"))
